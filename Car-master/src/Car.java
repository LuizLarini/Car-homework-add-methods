...
public class Car  {
    ...
    public void broken() {
        ...
        if (this.totalDistanceTravelled>1000){
    ...
    public void drive(int speed, int time) {
        ...
        int distance = 0;
        if(this.condition == "working"){
            distance = speed * time /2;
            fuel = fuel - (distance / 100) * LITRES_PER_100KM;
            totalDistanceTravelled =+ distance;
            if(totalDistanceTravelled >= 1000){
                System.out.println("You managed to break the car in the way, tou lose.");
            }
            else{
                System.out.println("You have arrived, you were driving for: " + distance + "Total distance is: " + totalDistanceTravelled + "You have: " + (totalDistanceTravelled - 1000) + "before the car breaks");
            }
        }
        else{
            System.out.println("The car is broken, you cannot drive it!");
        }
        //TODO: implement the method
    }
    ...
    public void tornado (int speed){
        int loopHolder = 0;
        int flyDistance;
        for (int loops=0; loops <= 15; loops++ ) {
            loopHolder =+ speed * loops;
        }
        flyDistance = loopHolder;
        System.out.println("Your car flew for" + flyDistance + "Kilometers, and now it is destroyed");
        condition = "broken";
    }
